...
class A5_Image {
	...
	public static function tags() {
		...
		$title_tag = __('Permalink to', 'rss-image-feed').' '.esc_attr($title);
		
		$image_alt = (isset($attachment)) ? trim(strip_tags(get_post_meta($attachment->ID, '_wp_attachment_image_alt', true))) : '';	
		$image_title = (isset($attachment)) ? trim(strip_tags($attachment->post_title)) : '';
	...
	public static function thumbnail($args) {
		...
		if (is_string($image_size)) :
			...
			$width = ('full' == $image_size) ? 9999 : $_wp_additional_image_sizes[$image_size]['width'];
