...
if ( function_exists( 'add_theme_support' ) ) {
	...
	// since 10.4 - please use Force Regenerate Thumbnails
	add_image_size( 'portfolio-list', 	1920, 750,  true  ); 	// portfolio - list

	add_image_size( 'blog-single', 		1200, 480,  true  ); 	// blog - single
	add_image_size( 'blog-navi', 		80,   80,   true  ); 	// blog - sticky navigation, widget
	add_image_size( 'blog-slider', 		280,   150,   true  ); 	// blog - sticky navigation, widget
...
if( ! function_exists( 'mfn_slider' ) )
{
	function mfn_slider( $id = false ){
		...
		if( $id || is_home() || is_category() || get_post_type() == 'page' || ( get_post_type() == 'portfolio' && get_post_meta( mfn_ID(), 'mfn-post-slider-header', true ) ) ){
 ...
 * --------------------------------------------------------------------------- */
if( ! function_exists( 'mfn_get_attachment_dimensions' ) )
{
	function mfn_get_attachment_dimensions( $image_id, $width_height, $echo = false ){
		$size = $return = false;
		
		$meta = wp_get_attachment_metadata( $image_id );
		if( is_array( $meta ) ){
			$return = $meta[ $width_height ];
		}
	
		if( $return && $echo ){
			$return = $width_height. '="'. $return .'"';
		}
