			...
			if( $animate ) $output .= '<div class="animate" data-anim-type="'. $animate .'">';
				$output .= '<div class="promo_box_wrapper promo_box_'. $position .'">';
					...
					$output .= '<div class="desc_wrapper">';
						if( $title )$output .= '<h2>'. $title .'</h2>';
						if( $content ) $output .= '<div class="desc">'. do_shortcode($content) .'</div>';
						if( $btn_link ) $output .= '<a href="'. $btn_link .'" id="btn-service" class="button button_left button_theme button_js" '. $target .'><span class="button_icon"><i class="icon-plus"></i></span><span class="button_label">'. $btn_text .'</span></a>';
					$output .= '</div>';
	...
	function sc_blog( $attr, $content = null )
	{
		...
		$output = '<div class="column_filters">';
			...
			if( $filters && in_array( $style, array('masonry','masonry tiles') ) && ! $category && ! $category_multi ){
				$output .= '<div id="Filters" class="isotope-filters" data-parent="column_filters">';
	...
	function sc_testimonials_list( $attr, $content = null )
	{
		...
		if ($query_tm->have_posts())
		{
			...
			while ($query_tm->have_posts())
			{
				...
				$output .= '<div class="item column mcb-column one-second '. $class .'">';
				
					if( has_post_thumbnail() ){
						$output .= '<div class="photo">';
							$output .= '<span class="testimonial-approved" ></span>';
							$output .= '<div class="image_frame no_link scale-with-grid no_border">';
								$output .= '<div class="image_wrapper">';
									$output .= get_the_post_thumbnail( null, 'full', array('class'=>'scale-with-grid' ) );
								$output .= '</div>';
							$output .= '</div>';
							//$output .= '<hr />';
							// $output .= '<a href="'. get_permalink() .'" id="btn-service" class="button button_left button_theme button_js kill_the_icon"><span class="button_icon"><i class="icon-plus"></i></span><span class="button_label">Ver Más</span></a>';
						$output .= '</div>';
					}
					...
					$output .= '<div class="desc">';
						...
						$output .= '<div class="blockquote">';
							$output .= '<p class="title-star" >'. get_the_title() .'</p>';
			...
			if( $animate ) $output .= '<div class="animate" data-anim-type="'. $animate .'">';
				if( $link ) $output .= '<a href="'. $link .'" '. $target .'>';
					...
					$output .= '<div class="list_right">';
						if( $title && $style != 4 ) $output .= '<h4><strong><span class="highlight" style="background-color:#fff; color:#142653;">'. $title .'</span></strong></h4>';
						$output .= '<div class="desc">'. do_shortcode( $content ) .'</div>';
					$output .= '</div>';
